...
public class CardsEditController : ControllerBase
{
    ...
    public async Task<IActionResult> UpdateCard(string referenceCode, [FromBody] VccEditRequest request)
    {
        var (_, isFailure, clientId, error) = _clientService.GetId();
        if (isFailure)
            return BadRequest(new ProblemDetails { Detail = error });

        var (isSuccess, _, editError) = await _vccService.Update(referenceCode, request, clientId);
        return isSuccess
            ? Ok()
